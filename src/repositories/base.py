...
class BaseRepository:
    ...
    async def add(self, data: BaseModel):
        try:
            ...
            data_hotel = await self.session.execute(add_stmt)
            res = data_hotel.scalars().one()
            if not res:
                return None
            return self.mapper.map_to_domain_entity(res)
        except IntegrityError as ex:
            logging.error(
                f"""Не удалось добавит данные в БД,
                 входные данные: {data},
            ...
            if isinstance(ex.orig.__cause__, UniqueViolationError):
                ...
                logging.critical(f"Незнакомая ошибка: {type(ex.orig.__cause__)}")
                raise ex
